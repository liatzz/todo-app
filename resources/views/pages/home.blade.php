        ...
        <div class="d-flex gap-3 px-3 flex-nowrap overflow-x-scroll overflow-y-hidden" style="height: 100vh;">
            @foreach ($lists as $list)  <!-- Iterasi untuk setiap list -->
                <div class="card flex-shrink-0 shadow-lg" 
                    ...
                    <!-- Bagian header setiap list (menampilkan nama dan tombol hapus) -->
                    <div class="card-header d-flex align-items-center justify-content-between shadow-sm" 
                        ...
                        <h4 class="card-title text-purple">🦋 {{ $list->name }}</h4> <!-- Nama list -->
                        <form action="{{ route('lists.destroy', $list->id) }}" method="POST" style="display: inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-sm p-0">
                                <i class="bi bi-trash fs-5 text-danger"></i> <!-- Ikon untuk menghapus daftar -->
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="card-body d-flex flex-column gap-2 overflow-x-hidden">
                        @foreach ($list->tasks as $task)  <!-- Iterasi untuk setiap task dalam list -->
                            <div class="card border-0 shadow-sm {{ $task->is_completed ? 'bg-secondary-subtle' : 'bg-light' }}">
                                <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
                                    ...
                                    <form action="{{ route('tasks.destroy', $task->id) }}" method="POST" style="display: inline;">
                                        ...
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm p-0">
                                            <i class="bi bi-x-circle text-danger fs-5"></i> <!-- Ikon untuk menghapus tugas -->
                                        </button>
                                    </form>
                                </div>
                                <div class="card-body">
                                    <p class="card-text text-truncate">{{ $task->description }}</p>  <!-- Deskripsi singkat tugas -->
                                </div>
                            </div>
                        @endforeach
